...
def ChangePassword(request):
    ...
    repairshop = tbl_repairshop.objects.get(id=request.session['sid'])
    dbpassword = repairshop.repairshop_password 
    if request.method == "POST":
        ...
        if dbpassword == old_password:
            if new_password == retype_password:
                repairshop.repairshop_password = new_password 
                repairshop.save()
                return redirect('Repairshop:MyProfile')
            else:
...
def SalesProfit(request):
    try:
        ...
        total_profit = total_sales * 0.9  # Deduct 10% admin cut
...
def chatpager(request,id):
    ...
    return render(request,"Repairshop/Chatr.html",{"user":user})
...
def ajaxchatr(request):
    ...
    return render(request,"Repairshop/Chatr.html")
...
def ajaxchatviewr(request):
    ...
    return render(request,"Repairshop/ChatViewr.html",{"data":chat_data,"tid":int(tid)})
...
def clearchatr(request):
    ...
    return render(request,"Repairshop/ClearChatr.html",{"msg":"Chat Deleted Sucessfully...."})
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if payment_method in ['card', 'cod']:
                request.session['payment_method_' + str(request_id)] = payment_method
                repair.repair_status = 5  
                repair.save()
                messages.success(request, f"Payment method set to {payment_method}. Proceed in View Repair Requests.")
                return redirect('Repairshop:ViewRepairRequest')
            else:
